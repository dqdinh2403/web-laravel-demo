...
@section('css')
	...
	<link rel="stylesheet" href="backend/css/colorbox.min.css" />
@endsection
...
@section('javascript2')
	...
	<script language="javascript">
        function deleteConfirm(stt,tensk){
            if(confirm("Bạn có chắc chắn muốn xóa hình " + stt + " của sự kiện '" + tensk + "' ?")){
                return true;
            }
            else{
                return false;
...
@section('content')
	<div class="page-content">
		<div class="page-header">
			<h1>Quản lý hình ảnh sự kiện "{{$sukien->sk_ten}}":</h1>
		</div><!-- /.page-header -->
		...
		<div class="row">
			<div class="col-xs-12">
				...
				<form id="formQuanLyHinhAnhSuKien" name="formQuanLyHinhAnhSuKien" method="POST" action="admin/sukien/hinhanh/{{$sukien->sk_ma}}" class="form-horizontal" role="form" enctype="multipart/form-data" > 
				    {{csrf_field()}}

				    <div class="form-group">
						<label class="col-sm-3 control-label no-padding-right" for="slQuyen"> Hiển thị trang chủ(*): </label>
						<div class="col-sm-9">
							<label>
								<input name="ckHienThiTrangChu" class="ace ace-switch ace-switch-6" type="checkbox"
									@if($sukien->sk_hienthitrangchu == 1)
										{{"checked"}}
									@endif
								 >
								<span class="lbl"></span>
					...
					<div class="form-group">
						<label class="col-sm-3 control-label no-padding-right" for="txtDiaChi"> Hình ảnh(*): </label>
						<div class="col-sm-4">
							<input type="file" id="id-input-file-2" name="fileHinhAnh" />
						</div>
					</div>
					...
					<div class="col-md-offset-3 col-md-9">
						...
						&nbsp; &nbsp; &nbsp;
						<button type="button" class="btn" name="btnHuy" id="btnHuy" onClick="window.location='admin/sukien/dsskyeshd'" >
				...
				<div class="col-md-offset-1 col-md-11">
					<ul class="ace-thumbnails clearfix">
						...
						<li>
							<a href="frontend/album/{{$ha->ha_tentaptin}}" data-rel="colorbox">
								<img width="150" height="150" alt="150x150" src="frontend/album/{{$ha->ha_tentaptin}}" />
								<div class="text">
									<div class="inner">{{"Hình " . $stt}}</div>
								</div>
							</a>

							<div class="tools tools-bottom">
								<a href="admin/sukien/xoahinh/{{$sukien->sk_ma}}/{{$ha->ha_ma}}" onclick="return deleteConfirm('{{$stt}}','{{$sukien->sk_ten}}')">
